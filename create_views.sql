...
CREATE VIEW AlunosEmCurso AS
SELECT ia.nome AS nome_aluno, a.email_aluno
FROM aluno a
JOIN matricula m ON a.num_matricula_aluno = m.numero_matricula
JOIN info_pessoais_aluno ia ON a.cpf_aluno = ia.cpf
...
CREATE VIEW AlunosPorTurma AS SELECT t.codigo_tur, COUNT(m.numero_matricula) AS qtd_alunos
FROM turma t
...
CREATE VIEW CursosComAlunos AS SELECT c.nome_curso, COUNT(m.numero_matricula) AS qtd_alunos
FROM curso c
...
CREATE VIEW TurmasComMenosAlunos AS SELECT t.codigo_tur, COUNT(m.numero_matricula) AS qtd_alunos
FROM turma t
LEFT JOIN matricula m ON t.codigo_tur = m.codigo_turma
...
SELECT ia.nome AS nome_aluno, c.nome_curso, a.data_inicio_aluno, c.duracao_curso, m.matricula_status
FROM aluno a
JOIN matricula m ON a.num_matricula_aluno = m.numero_matricula
